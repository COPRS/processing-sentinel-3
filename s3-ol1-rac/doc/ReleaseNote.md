...
# RS Add-on - S3 L1 OLCI RAC

 
 * [RS Add-on S3 L1 OLCI RAC](#rs-add-on---s3-l1-olci-rac)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 OL1-RAC Processor. It is processing the outputs from the S3 L0 workflow into S3 L1 products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-ol1-rac-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-ol1-rac-joborderxslt``, to be distinguishable from other tasktable configmaps.
