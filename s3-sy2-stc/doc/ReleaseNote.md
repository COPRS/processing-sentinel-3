...
# RS Add-on - S3 L2 SY2 NTC

 
 * [RS Add-on S3 L2 SY2 NTC](#rs-add-on---s3-l2-sy2-ntc)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SY2 NTC Processor. It is processing the outputs from the S3 L1 workflow (OL1 and SL1) into S3 L2 products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sy2-ntc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sy2-ntc-joborderxslt``, to be distinguishable from other tasktable configmaps.
