...
# RS Add-on - S3 L1 SR1 STC

 
 * [RS Add-on S3 L1 SR1 STC](#rs-add-on---s3-l1-sr1-stc)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SR1 STC Processor. It is processing the outputs from the S3 L0 workflow into S3 L1 STC products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sr1-stc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sr1-stc-joborderxslt``, to be distinguishable from other tasktable configmaps.
