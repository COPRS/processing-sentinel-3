...
# RS Add-on - S3 L2 SM2 LI STC

 
 * [RS Add-on S3 L2 SM2 LI STC](#rs-add-on---s3-l2-sm2-li-stc)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SM2-LI STC Processor. It is processing the outputs from the S3 L1 STC workflow into S3 L2 STC products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sm2-li-stc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sm2-li-stc-joborderxslt``, to be distinguishable from other tasktable configmaps.
