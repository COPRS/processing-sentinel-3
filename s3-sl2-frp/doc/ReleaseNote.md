...
# RS Add-on - S3 L2 SL2-FRP

 
 * [RS Add-on S3 L2 SL2-FRP](#rs-add-on---s3-l2-sl2-frp)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SL2-FRP Processor. It is processing the outputs from the S3 L1 workflow into S3 L2 products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sl2-frp-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sl2-frp-joborderxslt``, to be distinguishable from other tasktable configmaps.
