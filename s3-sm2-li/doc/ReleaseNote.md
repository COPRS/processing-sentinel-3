...
# RS Add-on - S3 L2 SM2 LI

 
 * [RS Add-on S3 L2 SM2 LI](#rs-add-on---s3-l2-sm2-li)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SM2-LI Processor. It is processing the outputs from the S3 L1 workflow into S3 L2 products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sm2-li-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sm2-li-joborderxslt``, to be distinguishable from other tasktable configmaps.
...
| Property | Details |
|----------|---------|
| ``app.execution-worker.process.level`` | Process level for the execution worker. Controls level specific logic. For S3 L0P: ``S3_L0`` |
