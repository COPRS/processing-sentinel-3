...
# RS Add-on - S3 L2 OLCI NTC

 
 * [RS Add-on S3 L2 OLCI NTC](#rs-add-on---s3-l2-olci-NTC)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 OL2 NTC Processor. It is processing the outputs from the S3 OL1 NTC workflow into S3 L2 NTC products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-ol2-ntc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-ol2-ntc-joborderxslt``, to be distinguishable from other tasktable configmaps.
