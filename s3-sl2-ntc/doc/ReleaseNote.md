...
# RS Add-on - S3 L2 SL2 NTC

 
 * [RS Add-on S3 L2 SL2 NTC](#rs-add-on---s3-l2-sl2-ntc)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SL2 NTC Processor. It is processing the outputs from the S3 L1 NTC workflow into S3 L2 NTC products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sl2-ntc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sl2-ntc-joborderxslt``, to be distinguishable from other tasktable configmaps.
