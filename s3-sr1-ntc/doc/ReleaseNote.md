...
# RS Add-on - S3 L1 SR1 NTC

 
 * [RS Add-on S3 L1 SR1 NTC](#rs-add-on---s3-l1-sr1-ntc)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SR1 NTC Processor. It is processing the outputs from the S3 L0 workflow into S3 L1 NTC products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sr1-ntc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sr1-ntc-joborderxslt``, to be distinguishable from other tasktable configmaps.
