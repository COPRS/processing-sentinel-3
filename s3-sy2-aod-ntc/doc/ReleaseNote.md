...
# RS Add-on - S3 L2 SY2 AOD NTC

 
 * [RS Add-on S3 L2 SY2 AOD NTC](#rs-add-on---s3-l2-sy2-aod-ntc)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SY2 AOD NTC Processor. It is processing the outputs from the S3 L1 workflow into S3 L2 products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sy2-aod-ntc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sy2-aod-ntc-joborderxslt``, to be distinguishable from other tasktable configmaps.
