...
# RS Add-on - S3 L1 SL1 NTC

 
 * [RS Add-on S3 L1 SL1 NTC](#rs-add-on---s3-l1-sl1)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SL1 Processor. It is processing the outputs from the S3 PUG workflow into S3 1 products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sl1-ntc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sl1-ntc-joborderxslt``, to be distinguishable from other tasktable configmaps.
