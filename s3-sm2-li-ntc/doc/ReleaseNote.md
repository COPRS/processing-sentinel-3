...
# RS Add-on - S3 L2 SM2 LI NTC

 
 * [RS Add-on S3 L2 SM2 LI NTC](#rs-add-on---s3-l2-sm2-li-ntc)
    * [Overview](#overview)
...
This add-on contains the configuration for the processing chain of the Sentinel-3 SM2-LI NTC Processor. It is processing the outputs from the S3 L1 NTC workflow into S3 L2 NTC products.
...
The config map will be created in kubernetes in the processing namespace and will be named ``s3-sm2-li-ntc-tasktables``, to be distinguishable from other tasktable configmaps.
...
This config map will be created in kubernetes in the processing namespace and will be named ``s3-sm2-li-ntc-joborderxslt``, to be distinguishable from other tasktable configmaps.
